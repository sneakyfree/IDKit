...
test.describe("Contract Management", () => {
    ...
    test("should show stats cards", async ({ page }) => {
        ...
        await expect(page.getByText(/Total Contract Value/i)).toBeVisible();
    });
    ...
    test("should have status filter buttons", async ({ page }) => {
        ...
        await expect(page.getByRole("button", { name: /signed/i })).toBeVisible();
    });
    ...
    test("should filter contracts by status", async ({ page }) => {
        ...
        await expect(page.getByRole("button", { name: /pending/i })).toHaveClass(/bg-purple/);
    });
});
